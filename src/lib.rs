//! A NFT RENT smart contract example using the Concordium Token Standard CIS2.
//!
//! # Description
...
/// The state for each loan.
#[derive(Serial, DeserialWithState, Deletable, StateClone)]
#[concordium(state_parameter = "S")]
struct LoanState<S> {
    /// The tokens owned by this address.
    owned_tokens: StateSet<ContractTokenId, S>,
    rent_token:StateSet<ContractTokenId, S>,
    rentTime:StateSet<SlotTime, S>,
    /// The address which are currently enabled as operators for this address.
    operators:    StateSet<Address, S>,
}

impl<S: HasStateApi> LoanState<S> {
    fn empty(state_builder: &mut StateBuilder<S>) -> Self {
        LoanState {
            owned_tokens: state_builder.new_set(),
            rent_token: state_builder.new_set(),
            rentTime: state_builder.new_set(),
            operators:    state_builder.new_set(),
        }
    }
}
...
struct State<S> {
    ...
    /// The state for each loan 
    stateloan:    StateMap<Address, LoanState<S>, S>,
    /// All of the token IDs
    all_tokens:   StateSet<ContractTokenId, S>,
    /// Map with contract addresses providing implementations of additional
    all_rent_tokens:   StateSet<ContractTokenId, S>,
    /// standards.
...
struct RentParams {
    ...
    _tokenId:TokenIdU32,
    _interestRate:TokenIdU32,
    _maxLoanAmount:TokenAmountU8,
    _loanCompleteTime:Timestamp,
    receiver_addr: AccountAddress

}

/// Parameter to fetch loan details
#[derive(Serial, Deserial, SchemaType)]
struct RentFetchParams {
    /// Owner of the newly minted tokens.
...
#[derive(Serial, Deserial, SchemaType)]
 struct RentStruct {
    ...
    lender_addr: Address,
    token_id:TokenIdU32,
    //    interestRate:TokenIdU32,
     max_loan_amount:TokenAmountU8,
    //  loan_complete_time:Timestamp,
     loan_id: u32
  }
//   static rentDetailArr : Vec<RentStruct> = Vec::new();
static RENT_DETAIL: Mutex<Vec<RentStruct>> = Mutex::new(Vec::new());
...
impl<S: HasStateApi> State<S> {
    ...
    fn empty(state_builder: &mut StateBuilder<S>) -> Self {
        State {
            ...
            stateloan:    state_builder.new_map(),
            all_tokens:   state_builder.new_set(),
            all_rent_tokens: state_builder.new_set(),
            implementors: state_builder.new_map(),
        }
    }


     /// Create new rent  with a given address as the owner
    fn create_rent(
        &mut self,
        token: ContractTokenId,
        loanid: ContractTokenId,
        timerent: Timestamp,
        owner: &Address,
        state_builder: &mut StateBuilder<S>,
    ) -> ContractResult<()> {
        ensure!(self.all_rent_tokens.insert(token), CustomContractError::TokenAlreadyRented.into());
        let mut owner_state =
            self.stateloan.entry(*owner).or_insert_with(|| LoanState::empty(state_builder));
        owner_state.owned_tokens.insert(token);

        owner_state.rent_token.insert(loanid);

        owner_state.rentTime.insert(timerent);
        Ok(())
    }
...
/// View function that returns all the addresses long with their tokens
/// testing.
#[receive(contract = "cis2_nft", name = "viewMinted", return_value = "ViewMintState")]
fn contract_view_mint<S: HasStateApi>(
    _ctx: &impl HasReceiveContext,
    host: &impl HasHost<State<S>, StateApiType = S>,
) -> ReceiveResult<ViewMintState> {
    let state = host.state();
    let mut inner_state = Vec::new();
    for (k, a_state) in state.state.iter() {
        let owned_tokens = a_state.owned_tokens.iter().map(|x| *x).collect();
    ...
    //let mut owner_state =
     //       state.state.entry(para_contract_token_idms.owner).or_insert_with(|| AddressState::empty(state_builder: &mut StateBuilder<S>));
    //let all_tokens = owner_state.owned_tokens.iter().map(|x| *x).collect();
    Ok(ViewMintState {
        state: inner_state,
    })
}

/// Rent owned tokens with a given address as the owner of these tokens.
...
#[receive(
    contract = "cis2_nft",
    name = "rent",
    parameter = "RentParams",
    return_value = "u64",
    error = "CustomContractError",
    payable,
    mutable
)]
fn create_loan<S: HasStateApi>(
    ctx: &impl HasReceiveContext,
    host: &mut impl HasHost<State<S>, StateApiType = S>,
    amount: Amount
  ) -> ContractResult<u64>{
    // Enforce creating future-dated loan
    let para_contract_token_idms: RentParams = ctx.parameter_cursor().get()?;
    let mut count = 0;
    let mut loan_id: u32 = 0;
    //if amount > Amount::from_micro_ccd( 2)  {
    //    return Err(ContractError::InvalidTokenId)}
    loan_id = NUM_LOANS + 1;
    let LOANID : ContractTokenId;
    LOANID = TokenIdU32(loan_id);
  
    let token_id : ContractTokenId = para_contract_token_idms._tokenId;
    let loantime : Timestamp = para_contract_token_idms._loanCompleteTime;
    let fromaddr : Address = para_contract_token_idms._from;
    let toaddre : Address = para_contract_token_idms._to;
  
    if host.invoke_transfer(&para_contract_token_idms.receiver_addr, amount).is_ok() {

    let ( state,  builder) = host.state_and_builder();

    ensure!(state.contains_token(&token_id), ContractError::InvalidTokenId);
  
    state.create_rent(token_id, LOANID, loantime, &fromaddr, builder)?;

    state.transfer(&token_id, para_contract_token_idms._maxLoanAmount, &para_contract_token_idms._from, &para_contract_token_idms._to, builder)?;

    let token_details  = RentStruct{
        token_owner: para_contract_token_idms._from,
        lender_addr: para_contract_token_idms._to,
        token_id:token_id,
        //    interestRate:TokenIdU32,
         max_loan_amount:para_contract_token_idms._maxLoanAmount,
        //  loan_complete_time:Timestamp::from_timestamp_millis(para_contract_token_idms._loanCompleteTim as u64),
         loan_id: loan_id
  };
  RENT_DETAIL.lock().unwrap().push(token_details);
